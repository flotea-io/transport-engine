<template>
	<div class="container">
		<header-menu activeIndex="4" @metamaskUpdated="(s)=>metamaskState = s" @getObject="(b) => blockchain = b">
		</header-menu>
		<div class="info" v-loading="loading">
	<div v-if="blockchain.userAddress!='' && !loading && routes.length == 0" class="text-center">
		<h1>
			Nie masz jeszcze dodanych tras
		</h1>
		<button @click="goToCreateRoute" class="btn-primary mb-4 mt-2">
			Dodaj trasę
		</button>
		<div class="info-box">
		<b>Wskazówka:</b> aby dodać trasę przejdź do zakladki tworzenia tras
		</div>
</div>
</div>
<div class="results">
	<div v-for="(route, routeIndex) in routes">
		<div class="top">
			Typ pojazdu: {{vehicleTypes[route.RouteType]}} 
			<span class="company">Firma: <a :href="route.AgencyUrl">{{route.AgencyName}}</a></span>
			<span type="text" class="blue-light-button" @click="showMap(route.RouteId)">Pokaż mapę</span>
		</div>
		<h3>
			{{route.FromLabel}} > {{route.ToLabel}}
			<span class="price">
				Cena: <span>{{route.Price}} {{route.CurrencyType}}</span>					
			</span>
			<br><small>{{route.RouteDesc}}</small>
		</h3>
		<el-collapse v-model="activeNames[routeIndex]">
			<el-collapse-item v-for="(times, time) in tickets[route.RouteId]" :name="time" v-bind:key="time">
				<template slot="title">
					<span class="time" v-html="getFormatedTime(time)"></span>
					<span class="tickets">Zakupione: {{times.length}} {{formatTickets(times.length)}}</span>
					<span>Cena {{times.length*route.Price}} FLT</span>
				</template>
				<div v-for="(count, buyer) in getBuyers(times)">
					<span class="buyer">
						{{buyer}}
					</span>
					<span class="count">
						zakupił: {{count}} {{formatTickets(count)}}
					</span>
				</div>
			</el-collapse-item>
		</el-collapse>
...
<no-metamask-modal text="Aby mieć wgląd we własne trasy, musisz być podłaczony do portfela Medtmask. Zaloguj sie lub link." :metamaskDialogVisible.sync="metamaskDialogVisible" />
...
export default {
	...
	methods: {
		goToCreateRoute:function(){
			location.href = '/carrier/create-route';
		},
		loadTickets: function(){
			let url = process.env.apiUrl+"/v1/search/carrier-routes";
			$.ajax({
				url: url,
				type: "POST",
				...
				this.routes = respond.Routes;
				this.loading=false;
			});
		},
		getFormatedTime(time){
			let t = moment.unix(time).utc();
			return t.format("DD. MM. YYYY") +" &nbsp; "+ t.format("H:mm");
		},
		...
		getBuyers: function(times){
			let buyers = {};
			for (var i = 0; i < times.length; i++) {
				if(typeof buyers[times[i].BuyerWallet] == "undefined")
					buyers[times[i].BuyerWallet] = 1;
				else
					buyers[times[i].BuyerWallet]++;
			}
			return buyers;
		}
	}
